...
const { server, html, js, vendor, css, img, packageJson } = paths;
 ...
 * -----  `copyVendorJsTask()`  -----
 * ----------------------------------
 * - Copia las dependencias JS del vendor al frontend.
 * @param {() => void} done - Funcion callback para finalizar si no hay carpeta vendor.
 */

export const copyVendorJsTask = (done) => {

    if (!fs.existsSync('public/assets/js/vendor'))
        return done();

    return src(vendor, { allowEmpty: true, encoding: false })
        .pipe(safePipe())
        .pipe(dest('dist/public/assets/js/vendor'));
};
